...
class Search extends Component {
  ...
  search(event) {
    ...
    axios
      .get(`/api/${this.state.searchType}?q=${searchTerm}`)
      .then(response => {
  ...
  render() {
    ...
    const blogResults = this.state.blogResults.map((blogResults, i) => (
      <BlogTile key={i} blog={blogResults} />
    ));
    const userResults = this.state.userResults.map((userResults, j) => (
      <UserTile key={j} user={userResults} />
    ));
